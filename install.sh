...
install -Dm755 "${REPO_DIR}/widgets/spotify/spotify_widget.py" "${DATA_DIR}/cosmic-spotify-widget/spotify_widget.py"
...
install -Dm644 "${REPO_DIR}/systemd/cosmic-spotify-widget.service" "${SYSTEMD_DIR}/cosmic-spotify-widget.service"
...
if [[ ! -e "${CONFIG_DIR}/cosmic-spotify-widget/config.json" ]]; then
  install -Dm644 "${REPO_DIR}/config/spotify.json" "${CONFIG_DIR}/cosmic-spotify-widget/config.json"
fi

systemctl --user daemon-reload
systemctl --user enable --now cosmic-weather-widget.service cosmic-calendar-widget.service cosmic-system-widget.service cosmic-spotify-widget.service

...
class SpotifyWidget(Gtk.Window):
    def __init__(self):
        ...
        for symbol, action in (("|◀", "Previous"), ("▶", "PlayPause"), ("▶|", "Next")):
            button = Gtk.Button(label=symbol); button.set_relief(Gtk.ReliefStyle.NONE); button.set_size_request(44, 36); button.get_style_context().add_class("control"); button.connect("clicked", self._control, action); controls.pack_start(button, False, False, 0)
